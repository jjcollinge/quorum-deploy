...
if [[ $exists == *"false"* ]]; then
    ...
    az storage table create --name $azure_storage_table >>logs/azure.log
fi
...
if [[ -z $current_bootnodes ]]; then
    ...
    az storage entity insert -t $azure_storage_table -e PartitionKey=$azure_partition_key RowKey=$azure_row_key Content=$current_bootnodes >>logs/azure.log
else
    current_bootnodes="$current_bootnodes,$bootnode_enode"
    echo "Updating bootnode registry with $current_bootnodes">>logs/start.log
    az storage entity replace -t $azure_storage_table -e PartitionKey=$azure_partition_key RowKey=$azure_row_key Content=$current_bootnodes >>logs/azure.log
fi

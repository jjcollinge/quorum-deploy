...
BASEDIR=$(pwd)
echo "Starting first member setup" | tee $BASEDIR/setup.log

if [[ $(dpkg -l | grep az | wc -l) == 0 ]]; then
  # Install azure cli 2.0
  echo "Installing Azure CLI 2.0" | tee -a $BASEDIR/setup.log
...
# Create the table and generate a sas token
echo "Generating SAS token for table storage" | tee -a $BASEDIR/setup.log
AzureTableStorageName=$AzureBlobStorageName
az storage table create -n networkbootnodes | tee -a $BASEDIR/setup.log
...
echo "Injecting values into config" | tee -a $BASEDIR/setup.log
...
echo "Copying files to local geth source" | tee -a $BASEDIR/setup.log
...
if [[ "$Rebuild" = true ]]; then
  echo "Building docker images" | tee -a $BASEDIR/setup.log
  docker-compose -f /opt/quorum-deploy/source/quorum-bootnode.yml build
fi

# Bring up docker containers
echo "Bringing up docker containers..." | tee -a $BASEDIR/setup.log
